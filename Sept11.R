...
#[No, can't start with numbers or underscore ]
...
commute <- c(my_commute, 20, 8, 25, 35)
commute

# Use the `length` function to store the number of observations in `n`.
n <-length(commute)
# assign and print at the same time by adding brackets
(n <-length(commute))
...
"hi"=="hi1"
typeof(TRUE)
# if the two variables are equal - "TRUE"
# if the two variables are different - "FALSE"
# TRUE and FALSE are one dimensional logical vectors
